...
void	cPlayer::Main(void)
{
	//--- カメラの計算 --------------------------------------------------------------------
	// カメラはプレイヤー座標とする
	d3.vecCamera.x = (m_vecPos.x - __SCR_W / 2 + BLK_SIZE / 2.f) /56.0f;
	d3.vecCamera.z = -(m_vecPos.y - __SCR_H / 2 + BLK_SIZE / 2.f) /56.0f;
	d3.vecCamera.y = 0.0f;

	//プレイヤーの向きより d3.vecEye を算出
	MTX		mtxRot;
	D3DXMatrixRotationYawPitchRoll(&mtxRot, R(-m_fRot), 0, 0);
	VEC3	a	= VEC3(0,	0,	1);
	D3DXVec3TransformCoord(&a, &a, &mtxRot);
	d3.vecEye	= d3.vecCamera - a;
	d3.vecEye.y = 0.0f;				// マイナスとすると足元を見る感じになる

	// ライトを必ず camera→eyeとする(やらないと壁が見づらくなります)
	d3.light.Direction = d3.vecEye -d3.vecCamera;
	
	//--- キャラ回転中 --------------------------------------------------------------------
	//
	//float	fRotTbl[4] = {90, 270, 0, 180};//90  270  0  180

	float	fRotTbl[4] = {90, 0, 270, 180};//90  270  0  180
	...
	if(m_iMoveTime == 0)
	{
		...
		{
			m_iMoveTime = MOVE_TIME;
			switch(m_iDir)			// 各方向に前進
			{
			case 3:	m_vecSpeed.x = 0;			 m_vecSpeed.y = -MOVE_SPEED; break;
			case 2: m_vecSpeed.x = -MOVE_SPEED;  m_vecSpeed.y = 0;			 break;
			case 1:	m_vecSpeed.x = 0;			 m_vecSpeed.y =  MOVE_SPEED; break;
			case 0: m_vecSpeed.x =  MOVE_SPEED;  m_vecSpeed.y = 0;			 break;
			}
		}
		// 左右で向きを変える(& 3 は % 4 と同意)
		if(pad.bKeyClick[KEY_LEFT   ]){ m_iDir = (m_iDir + 1) & 3;}
		if(pad.bKeyClick[KEY_RIGHT  ]){ m_iDir = (m_iDir - 1) & 3;}
