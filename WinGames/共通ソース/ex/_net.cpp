...
static	BOOL	bSend_;						// true : データ送信要求
...
BOOL	SockSend(int iNum)
{
	...
	if(!bSend_) return false;				// net_send による要求があった場合のみ対応する
...
| <<< サーバーが接続準備をする >>>
*------------------------------------------------------------------------------*/
void	net_server_connect_preparation(void)
{
	if(bServer)
	{
		if(net_listen_preparation()){ iSockMode = SOCK_WAIT;}
